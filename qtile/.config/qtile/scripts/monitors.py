...
options = ['Duplicate', 'Extend to the right', 'Extend to the left', 'Primary only', 'Secondary only']

# Get the names of the connected monitors
xrandr_process = subprocess.Popen(['xrandr'], stdout=subprocess.PIPE, encoding='utf-8')
...
if selected_option == 'Duplicate':
    ...
    xrandr_process = subprocess.Popen(['xrandr', '-q', '--verbose'], stdout=subprocess.PIPE, encoding='utf-8')
    xrandr_output, _ = xrandr_process.communicate()

    secondary_resolution = re.findall(r'HDMI-1 connected.*?(\d+x\d+).*?\n', xrandr_output)[0]

    # Set the command to duplicate the primary monitor onto the secondary monitor
    command = f'xrandr --output {connected_monitors[1]} --mode {secondary_resolution} --same-as {connected_monitors[0]}'
    ...
    command = f'xrandr --output {connected_monitors[0]} --auto --scale 0.75x0.75 --output {connected_monitors[1]} --auto --right-of {connected_monitors[0]}'
    ...
    command = f'xrandr --output {connected_monitors[0]} --auto --scale 0.75x0.75 --output {connected_monitors[1]} --auto --left-of {connected_monitors[0]}'
elif selected_option == 'Primary only':
    # Set the command to use only the primary monitor
    command = f'xrandr --output {connected_monitors[0]} --auto --scale 0.75x0.75 --output {connected_monitors[1]} --off'
...
xrandr_process = subprocess.Popen(command.split(), stdout=subprocess.PIPE, encoding='utf-8')
xrandr_output, _ = xrandr_process.communicate()
